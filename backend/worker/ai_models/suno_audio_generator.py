...
def iniciar_geracao(
    ...
    """Submete um pedido de geracao ao Suno e devolve o taskId."""
    if not API_KEY:
        raise RuntimeError("SUNO_API_KEY nao esta definida. Configure no backend/.env")
    ...
    payload = {
        ...
        "style": style,
        "title": title,
        "callBackUrl": CALLBACK_URL or "https://webhook.site/noop",
    }
    ...
    try:
        ...
        print(f"Erro de rede ao iniciar geracao: {e}")
    return None
...
def iniciar_cover(
    ...
    """Submete um pedido de cover ao Suno e devolve o taskId.

    O Suno descarrega o audio a partir de upload_url (presigned URL do R2),
    processa-o e gera uma versao nova no estilo indicado em style.
    """
    if not API_KEY:
        raise RuntimeError("SUNO_API_KEY nao esta definida. Configure no backend/.env")

    if not upload_url or not upload_url.startswith(("http://", "https://")):
        raise ValueError("upload_url invalido. Deve ser uma URL publica http(s).")
...
def guardar_ficheiro(url: str, caminho: str) -> bool:
    """Descarrega um ficheiro de uma URL e guarda-o no caminho indicado."""
